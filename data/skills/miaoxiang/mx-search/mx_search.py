...
# 默认输出目录: ~/.openclaw/data/output/miaoxiang/
...
def default_miaoxiang_output_dir() -> Path:
    override = os.environ.get("MIAOXIANG_OUTPUT_DIR", "").strip()
    if override:
        return Path(override).expanduser()
    return Path.home() / ".openclaw" / "data" / "output" / "miaoxiang"
...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        description="妙想资讯搜索（东方财富妙想 API）",
    )
    parser.add_argument("query", help="搜索问句（建议用引号包裹）")
    parser.add_argument(
        "output_dir",
        nargs="?",
        default=str(default_miaoxiang_output_dir()),
        help="输出目录（可选，默认 ~/.openclaw/data/output/miaoxiang 或环境变量 MIAOXIANG_OUTPUT_DIR）",
    )

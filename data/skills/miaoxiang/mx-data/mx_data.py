...
def default_miaoxiang_output_dir() -> Path:
    """妙想技能统一数据输出目录：~/.openclaw/data/output/miaoxiang（可用环境变量 MIAOXIANG_OUTPUT_DIR 覆盖）"""
...
def main():
    ...
    if len(sys.argv) < 2:
        ...
        print(f"默认输出目录: {default_miaoxiang_output_dir()}")
    ...
    if len(sys.argv) >= 3:
        ...
        output_dir = default_miaoxiang_output_dir()
